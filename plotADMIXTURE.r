...
tiff(file=paste0(opt$outPrefix,".tiff"),width = 2000, height = 1200,res=200)
 ...
 # Plot minK
 bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=loonR::get.palette.color("sp1", minK, 0.7),xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
 axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6)
 # Plot higher K values
 if(maxK>minK)lapply(2:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=loonR::get.palette.color("sp1",x+1,0.7), xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces))
 axis(1,at=c(which(spaces==0.5),bp[length(bp)])-diff(c(1,which(spaces==0.5),bp[length(bp)]))/2,
     labels=unlist(strsplit(opt$populations,",")))
dev.off()

#pdf(file=paste0(opt$outPrefix,".pdf"),width = 2000, height = 1200)
