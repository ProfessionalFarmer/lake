...
cINPUT_DIR  = "/master/zhu_zhong_xu/work/ProstaticCancer/data/Test_N2304161_80-1164162049_RNA_2023-05-17/combined"
cOUTPUT_DIR = "/master/zhu_zhong_xu/work/ProstaticCancer/analysis/20230528Test3Samples"
cDATE = "01_05_2023"
cAdapter = "TGGAATTCTCGGGTGCCAAGG"

print("Input directory", cINPUT_DIR)

(SRR,) = glob_wildcards(cINPUT_DIR  + "/{sample}_1.fq.gz")
...
rule all:
   ...
   #     expand(cOUTPUT_DIR + "/{sample}/{sample}.analysis.done",sample = SRR),
        ...
        expand(cOUTPUT_DIR + "/{sample}/{sample}_clean_1.fq.gz", sample = SRR),
    output:
...
rule fastqc_raw:
    input:
        R1 = cINPUT_DIR  + "/{sample}_1.fq.gz"
    resources:
        mem_mb = 40000,
        runtime_s = 34560
    output:
        FastqcDone = cOUTPUT_DIR + "/{sample}/done.fastqc_raw.txt",
        CutadaptDone = cOUTPUT_DIR + "/{sample}/done.cutadapt_raw.txt",
        FastqClean = cOUTPUT_DIR  + "/{sample}/{sample}_clean_1.fq.gz"
    log: cOUTPUT_DIR + "/{sample}/cutadapt.log"
    benchmark: cOUTPUT_DIR + "/{sample}/benchmark.fastqc_raw.txt"
    params:
        TITLE = "{sample}",
    ...
    shell:
        ...
        fastqc -o {params.SAMPLE_DIR} -t {threads} {input.R1} 2>&1 >> {log} && touch {output.FastqcDone}

        cutadapt --minimum-length 17 -q 20 --trim-n -j {threads} -a {params.adapterSeq} -o {output.FastqClean}  {input.R1} 2>&1 > {log} && touch {output.CutadaptDone}
        
        # cutadapt -j 5 -u 4 -u -4 -o APC80.cl  ean.R1.fastq.gz  APC80.clean.notrim.R1.fastq.gz 2>&1 > APC80.cutadapt.trim.log
        """
...
rule fastqc_clean:
    input:
        R1 = cOUTPUT_DIR  + "/{sample}/{sample}_clean_1.fq.gz"
    resources:
        mem_mb = 40000,
        runtime_s = 34560
    output:
...
rule mirdeep2:
    ...
    params:
        ...
        SAMPLE_DIR = cOUTPUT_DIR + "/{sample}",
        Adapter = cAdapter,
        Bowtie_Index = "/master/zhu_zhong_xu/ref/hg38/gencode/GRCh38.miRDeep2",
        Precursor = "/master/zhu_zhong_xu/ref/miRBase22/hairpin_ref.fa",
        Mature = "/master/zhu_zhong_xu/ref/miRBase22/mature_ref.fa",
        Date = cDATE,
    threads: 10
    shell:
        """
