...
option_list <- list(
  ...
  make_option(c("-m", "--minK"),
    type = "integer", default = 2,
    help = "minimum K value", metavar = "integer"
  ),
  make_option(c("-l", "--populations"),
    type = "character", default = NULL,
...
### ZZX
opt$prefix = "bee.RAD"
opt$outPrefix = "/data0/Zhongxu/BeeProject/analysis/20220613GATK/4.strucutre"
opt$maxK = 5 
opt$populations="A.Lineage,C.Lineage,Dongbei,L.Lineage,M.Lineage,O.Lineage,U.Lineage,Y.Lineage"
opt$infofile = "/data0/Zhongxu/BeeProject/analysis/20220613GATK/4.strucutre/bee.RAD.list"
...
minK <- opt$minK
maxK <- opt$maxK
tbl <- lapply(minK:maxK, function(x) read.table(paste0("/data/home2/Zhongxu/work/LinBee/20210111/structure/",prefix, ".", x, ".Q")))
...
# svg(file = paste0(opt$outPrefix, ".svg"))
# 
# par(mfrow = c(maxK - 1, 1), mar = c(0, 1, 0, 0), oma = c(2, 1, 9, 1), mgp = c(0, 0.2, 0), xaxs = "i", cex.lab = 1.2, cex.axis = 0.8)
# 
# # Plot minK
# bp <- barplot(t(as.matrix(tbl[[1]][order(labels$n), ])), col = loonR::get.palette.color("aaas", minK), xaxt = "n", border = "white", ylab = paste0("K=", minK), yaxt = "n", space = spaces)
# axis(3, at = bp, labels = labels$ind[order(labels$n)], las = 2, tick = F, cex = 0.6)
# 
# # Plot higher K values
# if (maxK > minK) lapply(2:(maxK - 1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n), ])), col = loonR::get.palette.color("aaas",x+1), xaxt = "n", border = "white", ylab = paste0("K=", x + 1), yaxt = "n", space = spaces))
# axis(1,
...
cv.error  <- read.table("/data/home2/Zhongxu/work/LinBee/20210111/structure/bee.RAD.cv.error",
                        header = F, sep = ' ', quote = ''  
                        )
colnames(cv.error) <- c("K","CV error")
library(ggpubr)

ggline(cv.error, x = "K", y = "CV error", ylim = c(0.5,0.56)
